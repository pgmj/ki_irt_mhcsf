...
```{r}
pmatr1 <- check.pmatrix(df)
summary(pmatr1)
#plot(pmatr1)
```

```{r}
restsc1 <- check.restscore(df)
summary(restsc1)
#plot(restsc1)
```
...
The violation found in restscore.com\$results\[\[31\]\]\[\[3\]\] is also
shown in the graph because the thick solid line and the thick dashed
line intersect.
:::
